...
//       глобали, ни чтения секретов. Реестр разрешённых файлов **ПУСТ**: в v1-контуре
//       провайдер один и он `requiresNetwork: false`. Пополняется задачей `V-06` (живой
//       ElevenLabs), и живой прогон делается ВНЕ `fixtures/` (roadmap §3, решение владельца 8);
...
/**
 * Файлы `packages/voice/src/**`, которым сеть и ключи разрешены. **ПУСТ в v1.**
 * Адрес пополнения — `V-06` (провайдер ElevenLabs).
 */
...
describe('**V9** (а) — в `packages/voice/src/**` нет ни сети, ни ключей', () => {
  ...
  it('реестр разрешённых файлов ПУСТ — и это утверждение, а не умолчание', () => {
    expect(
      ALLOWED,
      'В v1-контуре провайдер один и он `requiresNetwork: false`. Непустой реестр означает, ' +
        'что появился сетевой путь; его адрес — `V-06`, и живой прогон делается вне `fixtures/`.',
    ).toEqual([]);
  });
