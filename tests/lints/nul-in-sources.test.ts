...
// ОДНА строка; `ENV-01`, 2026-08-31 — две; `E-02`, 2026-08-31 — ЧЕТЫРЕ.)* Правило про исключения при этом исполнено буквально, а не обойдено: файл
...
const ALLOWED: readonly string[] = [
  ...
  // **Шрифт гейта `captionEmphasis@1`** (`ENV-01`, долг №187): `DejaVuSans-Bold.ttf`, 705684
  // байта, sha `d1c3ff99…287b`, 86278 литеральных NUL. Он ОТЛИЧАЕТСЯ от соседа сверху: тот
  // производный, его порождает билдер из литерала, — этот ИСХОДНЫЙ, байты пришли из
  // системного пакета `fonts-dejavu-core` и никем не порождаются. ПОЧЕМУ ОН НЕ МОЖЕТ БЫТЬ
  // ТЕКСТОВЫМ — три причины, и ни одна не про удобство:
  //   1. это шрифт: его байты читает `materialize.ts`, кладёт в каталог композиции файлом
  //      `fonts/<sha>.ttf` и считает их `sha256` в `bundle.hash`; текстовая форма дала бы в
  //      композиции другой файл, другой хэш и, значит, устаревшими ДЕСЯТЬ записей гейта;
  //   2. расширение определяется по МАГИЧЕСКИМ БАЙТАМ (`extensionOf`, правило R3) — у TrueType
  //      это `00 01 00 00`, то есть первый же NUL и есть опознавательный знак формата;
  //   3. подделка измерена и отвергнута (долг №187): с 12-байтовым `TTF_STUB` `@font-face` не
  //      грузится, `font-display: block` держит текст невидимым ~3 с, и в кадры въезжает
  //      ТАЙМЕР вместо типографики — гейт мерил бы гонку загрузки шрифта.
  // Лицензия лежит рядом файлом `DejaVuSans-Bold.LICENSE.txt` (Bitstream Vera, разрешает
  // распространение); в ней NUL нет, и в этом списке ей делать нечего.
  'packages/renderer-hyperframes/gate-requests/assets/DejaVuSans-Bold.ttf',
  // **ДВА СЛОЯ ЗАПРОСОВ `parallax25@1`** (`E-02`): 32×32 PNG с АЛЬФОЙ — дальний непрозрачен
  // целиком, ближний вырезан ромбом. Природа у них та же, что у `pattern-32.png` сверху
  // (производные: их порождает и сверяет побайтово `test/gate-requests.test.ts` из литералов
  // `PARALLAX_LAYER_PNGS`), и три причины «почему не текстом» те же дословно — байты едут в
  // композицию и в `bundle.hash`, расширение опознаётся магическими байтами (R3), подделка
  // мерила бы не то. Четвёртая причина СВОЯ и её нет у соседей: **предмет этих файлов —
  // альфа-канал**. PNG без альфы (и любая текстовая форма) дал бы гейт на стопке
  // непрозрачных прямоугольников — то есть измерение композитинга, которого у параллакса
  // нет, и ложно-зелёный класса №164 в четвёртый раз.
  'packages/renderer-hyperframes/gate-requests/assets/parallax-far-32.png',
  'packages/renderer-hyperframes/gate-requests/assets/parallax-near-32.png',
];
