...
describe('NUL в исходниках запрещён (`CP-04fix`, 2026-08-27)', () => {
  ...
  // **ЗОНА ИСКЛЮЧЕНИЯ НАЙДЕНА И НЕ ШИРЕ, ЧЕМ ОБЪЯВЛЕНА** (`TPL-01a`). Паттерн без этого
  // утверждения — обещание: опечатка в регулярке дала бы либо мёртвое исключение (и тогда
  // тест краснеет на шрифте гейта), либо слишком широкое (и тогда молчал бы весь пакет).
  it('исключение накрывает ровно ассеты запросов гейта и ничего сверх', () => {
    const files = scannedFiles();
    const covered = files.filter((file) => allowed(file)).sort();
    // Пять файлов: три копии шахматки, шрифт с лицензией, два слоя параллакса.
    ...
    // Ни один файл вне `gate-requests/assets/` под исключение не подпадает.
    expect(covered.every((file) => file.includes('/gate-requests/assets/'))).toBe(true);
    // И ни один ИСХОДНИК шаблона — тем более: спек и запросы обязаны быть текстом.
