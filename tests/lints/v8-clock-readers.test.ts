...
//
// ЧТО ЗАПИСАНО ЗАРАНЕЕ. Когда `L-01` добавит файл, он появится в двух местах — здесь и
// в `eslint.config.js` (как `packages/core-model/src/anchors/mint.ts` для `node:crypto`), и
// этот файл обязан получить проверки «исключение узкое» (сосед по каталогу под правилом
// остаётся) и «исключение не мёртвое» (файл действительно читает часы). Пока их нет, потому
// что нечего проверять: пустой реестр проверяется поведением — ниже пункт (в).
//
