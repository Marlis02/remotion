...
describe('`H-06` — реестр реализаций совпадает с библиотекой спеков', () => {
  it('шесть реализаций, версия реестра не менялась', () => {
    // ~~Пять~~ ШЕСТЬ: `grade@1` (`E-07`). Число стоит ЛИТЕРАЛОМ, а не `TEMPLATE_LIBRARY.length`,
    // — иначе сверка «столько, сколько получилось» была бы зелёной при любом наполнении.
    ...
    // (`E-07`): ни один существующий сегмент `grade@1` не зовёт.
...
describe('`H-06` — `params` фикстуры проходят схему СПЕКА (реализация написана под них)', () => {
  ...
  // разница — ровно перечисленная ниже.
  ...
  it('фикстура ⊆ реестр, а разница — ровно `grade@1` и ничего сверх', () => {
    ...
    // И счёт сходится: пять вызовов фикстуры + одно названное имя = шесть реализаций.
...
describe('**П1-а** — форма DOM, на которую опираются реализации (двусторонний охранник)', () => {
  ...
  it('реализации читают ровно эти имена — и ни одного лишнего окна из `clip.frames`', () => {
    ...
    // `captionEmphasis@1` и `grade@1` окном пользуются, а `bed@1` — реализация-отказ
    // (аудио-домен, долг №189), и кадров у него нет по построению. Поэтому проверяется не
    // «каждый читает», а «кто читает — читает модельной парой имён»; пустой список читателей
    // был бы зелёным охранником ни о чём, и от этого стережёт счёт.
    // *(число: 4 → 5, `E-07` — `grade@1` открывает и закрывает окно двумя `set`, как `still@1`.)*
