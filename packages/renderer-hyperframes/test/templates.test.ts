...
describe('`H-06` — реестр реализаций совпадает с библиотекой спеков', () => {
  it('девять реализаций, версия реестра не менялась', () => {
    // ~~Пять~~ ~~шесть~~ ~~СЕМЬ~~ ~~ВОСЕМЬ~~ **ДЕВЯТЬ**: `grade@1` (`E-07`), `parallax25@1`
    // (`E-02`), `video@1` (`VID-02a`, 2026-09-11), `kineticType@1` (`KT-01`, 2026-09-12).
    // Число стоит
...
describe('`H-06` — `params` фикстуры проходят схему СПЕКА (реализация написана под них)', () => {
  ...
  // **РАЗНИЦА МЕЖДУ РЕЕСТРОМ И ФИКСТУРОЙ ВЫЧИСЛЯЕТСЯ** *(изменено: `TPL-01a`, 2026-09-09;
  // долг №222 закрыт — вторая копия списка была ЗДЕСЬ)*.
  //
  // ~~Решение владельца `E-07`: разница названа поимённо, чтобы седьмой молча добавленный
  // шаблон краснел ровно как раньше.~~ Реестр реализаций перестал быть ручным массивом: он
  // ПРОИЗВОДНЫЙ от листинга `src/templates/<id>@<N>/` (`scripts/gen-template-registry.mjs`), и
  // «молча добавить» в него нечего — папка без запуска генератора красит
  // `tests/lints/template-registry-generated.test.ts`, а запуск кладёт строку в дифф. Охранник
  // не снят, он переехал в механизм.
  //
  // Направление «фикстура ⊆ реестр» осталось ЖЁСТКИМ: вызов без реализации — красный тест.

  it('фикстура ⊆ реестр, а разница — реализации, которых фикстура не зовёт', () => {
    ...
    // Направление 2: счёт сходится — реестр не может «потерять» реализацию, оставшись
    // равным по длине.
    const extra = known.filter((name) => !used.includes(name));
    expect(used).toHaveLength(5);
    expect(known).toHaveLength(used.length + extra.length);
    // И оба реестра описывают ОДИН каталог: имена реализаций совпадают с именами спеков.
...
describe('**П1-а** — форма DOM, на которую опираются реализации (двусторонний охранник)', () => {
  ...
  it('реализации читают ровно эти имена — и ни одного лишнего окна из `clip.frames`', () => {
    ...
    // ДЛИТЕЛЬНОСТЬ хода; 6 → 7, `KT-01` — `kineticType@1` берёт из окна ОБЕ границы: начало
    // задаёт отсчёт счётчика и набора, конец — гашение последнего куска.)*
