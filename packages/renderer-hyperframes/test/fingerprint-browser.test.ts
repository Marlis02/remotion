...
describe.skipIf(NO_BROWSER)('версия Chrome — от БИНАРЯ, который запускается (№156)', () => {
  ...
  it('версия — из `--version` бинаря, а не из имени каталога кэша', { timeout: 120_000 }, () => {
    ...
    // Она стоит здесь как ИЗМЕРЕНИЕ и как вход для проверки ниже.
    expect(CHROME).not.toBeNull();
  });

  it('путь резолвера и версия бинаря — про одну и ту же установку', { timeout: 120_000 }, () => {
    // ИСТОРИЯ ЭТОГО ТЕСТА ВАЖНА, поэтому она здесь. В `H-03` он был НАХОДКОЙ: `browserPath`
    // тогда спрашивал путь у рендерера (`hyperframes browser path`), и тот отдавал
    // `~/.cache/puppeteer/…/linux-152.0.7977.42/…` при лежащей рядом пришпиленной
    // `~/.cache/hyperframes/…/linux-152.0.7928.2/…` — то есть отпечаток мерил не тот бинарь,
    // который запустится (долг №160).
    //
    // ПОСЛЕ `H-05` тот же ассерт стал ОХРАННИКОМ ИСПРАВЛЕНИЯ, а не свидетельством дефекта:
    // `browserPath` больше ничего не спрашивает у CLI, а выбирает по нашему корню, и вернуть
    // чужую установку он не может по построению. Красный здесь теперь означает, что резолвер
    // и запускаемый бинарь снова разъехались. Утверждение при этом НЕ про конкретную
    // версию — оно про то, что отпечаток описывает ТУ установку, которую вернул резолвер.
    const value = probe().fields['chrome'];
