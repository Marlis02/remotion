...
describe('шаблон без реализации — отказ ДО того, как на диск лёг байт', () => {
  ...
  // `parallax25@1`.)*
  // *(дополнено: `VID-02a`, 2026-09-11 — ВОСЕМЬ: `video@1`; `KT-01`, 2026-09-12 — ДЕВЯТЬ:
  // `kineticType@1`.)* Число стоит литералом, а не
  // `.length`, по той же причине, что у соседей: сверка «столько, сколько получилось» была бы
  // зелёной при любом наполнении.
  it('продакшн-реестр НАПОЛНЕН девятью, и `solid@1` в нём нет', () => {
    expect(rendererTemplates.templates).toHaveLength(9);
