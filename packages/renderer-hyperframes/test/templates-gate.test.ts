// **ЖИВОЙ ГЕЙТ V13 НА ~~ЧЕТЫРЁХ~~ ПЯТИ ВИЗУАЛЬНЫХ ШАБЛОНАХ: `draftHalf`, N = 3.**
// *(четыре — `H-06`; пятый, `grade@1`, — `E-07`, 2026-08-31.)*
//
...
//
// **`grade@1` СНИМАЕТСЯ НА СМЕШАННОМ ЗАПРОСЕ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И `kenburns@1`** (`E-07`).
// Он красит `backdrop`, то есть то, что лежит НИЖЕ него; над пустотой backdrop пуст, и гейт
// мерил бы воспроизводимость ничего. Основание — тот же `still@1` на той же картинке 32×32.
// **И `params` его несут ЗЕРНО, а не ноль:** зерно — самая дорогая и самая сомнительная часть
// шаблона (детерминизм `feTurbulence` в headless Chrome — `INFERENCE`), и гейт без него
