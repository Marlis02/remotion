...
describe('живая проба: бинари', () => {
  ...
  it('поле Chrome: либо измеренная версия, либо `absent` с ПРИЧИНОЙ (форма, не значение)', { timeout: 60_000 }, () => {
    ...
    if (value?.state === 'absent') {
      ...
      //
      // ПЕРЕЦЕЛЕНО `H-05fix`. До `H-05` причина называла МЕХАНИЗМ поиска (`browser path` —
      // подпроцесс, у которого спрашивали путь), и ассерт стоял на подстроке `browser path`.
      // `H-05` этот механизм убрал: путь выбирает наш резолвер по пришпиленному корню
      // (долг №160), и текст причины стал другим. Ветка исполняется ТОЛЬКО там, где браузера
      // нет, поэтому на машине разработчика расхождение было невидимо, а на приёмке дало
      // красный. Утверждение при этом НЕ ослаблено: проверяется то же самое — причина ведёт
      // человека к команде установки, — плюс появилось третье, которого раньше не было:
      // сказано, ПОЧЕМУ лежащий рядом чужой Chrome не годится (иначе отказ выглядел бы
      // поломкой на машине, где Chrome «вот же он»).
      expect(value.reason).toContain('preflight');
      expect(value.reason).toContain('не является браузером');
      expect(value.reason).toContain('$HOME/.cache/hyperframes');
    } else {
...
describe('живая проба: отпечаток', () => {
  it(
    ...
    // Явный таймаут: проба — ТРИ подпроцесса (`chrome --version`, `ffmpeg -version`,
    // `ffprobe -version`), и умолчание vitest в 5 с их не покрывает. Четвёртым был
    // `hyperframes browser path` (ИЗМЕРЕН `H-03` в 2–3 с); `H-05` его убрал — путь выбирает
    // резолвер по каталогу, без подпроцесса, — но запас оставлен: две пробы подряд занимали
    // до 12 с, и подбирать таймаут заново под каждую правку дороже, чем держать его с запасом.
    60_000,
  );
