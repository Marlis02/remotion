...
// НАПОЛНЕН: ~~пять~~ ШЕСТЬ реализаций.** *(дополнено: `E-07`, 2026-08-31 — `grade@1`.)*
...
// поэтому в рантайме стрелка одна: реестр тянет шесть файлов, они его — нет.
...
/**
 * Продакшн-реестр реализаций — ~~**пять единиц** (`H-06`)~~ **ШЕСТЬ единиц** (`E-07`).
 *
 ...
 * зовёт вовсе. Он ни на что не влияет (реестр адресует по имени), и именно поэтому взят тот
 * же: любой другой пришлось бы объяснять.
 *
 ...
 * — измерено `E-07` побайтовой сверкой восьми файлов `gate-requests/`.
 */
