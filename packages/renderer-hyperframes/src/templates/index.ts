...
// НАПОЛНЕН: пять реализаций.** `H-01` доказывал СКВОЗНОЙ ПУТЬ одним синтетическим шаблоном,
...
// поэтому в рантайме стрелка одна: реестр тянет пять файлов, они его — нет.
...
/**
 * Продакшн-реестр реализаций — **пять единиц** (`H-06`).
 *
 ...
 * `fixtures/minimal/direction/01-intro.yaml`. Он ни на что не влияет (реестр адресует по
 * имени), и именно поэтому взят тот же: любой другой пришлось бы объяснять.
 */
