...
export type RenderRule =
  ...
  | 'ADR-0008 композиция'
  /**
   * R12 — сборка сегмента не стартует без записи гейта для пары (профиль,
   * `engineFingerprint`). Правило Charter V13; вход — `assertBuildMayStart` из
   * `templates-spec` (`TS-01`), вызывающий — `renderSegment` (`H-04`).
   *
   * Этим же правилом отвечает ОТСУТСТВИЕ решения: у `renderSegment` нет умолчания «рендерить
   * без проверки», и вызов без поля `gate` — отказ, а не тихий проход (решение владельца
   * `H-04`, вопрос 2).
   */
  | 'R12'
