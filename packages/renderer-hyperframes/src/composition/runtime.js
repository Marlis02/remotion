...
(function () {
  ...
  //
  // **ВИДИМОСТЬ ГРУПП ЖИВЁТ ЗДЕСЬ, А НЕ В ШАБЛОНЕ** *(добавлено: `H-06`, 2026-08-29, решение
  // владельца — вариант R2.)* ИЗМЕРЕНО (`H-06`, живой гейт): `mount` клипа исполняется в цикле
  // ВЫШЕ, то есть раньше, чем этот слой вообще создан, — `captionEmphasis@1` не мог найти
  // `#captions` и падал «оформлять нечего». Значит показ групп шаблону эмфазы физически
  // недоступен. Он и не его: субтитры приходят треком `IR.captions`, а не вызовом шаблона
  // (V3), и в сегменте без клипа эмфазы — а таких большинство — они обязаны показываться
  // ровно так же. Шаблон оформляет ВНЕШНИЙ ВИД (инъекцией `<style>`, от порядка не зависящей);
  // ВРЕМЯ принадлежит треку.
  //
  // Побочно закрыта находка: `data-start`/`data-duration` групп до этой правки не читал НИКТО
  // — они были мёртвыми данными, а все группы сегмента показывались одновременно, друг на
  // друге. На таком входе **R13** (`H-02`) не измерил бы ни одной смены строки.
  //
  // ОКНА — ТЕ ЖЕ `group.frames`, что уже посчитаны выше; формы `clip.frames` эта правка не
  // касается ни в какую сторону (долг №168 — адрес `L-01`).
  ...
  // Таймлайн субтитров — ОБЁРНУТЫЙ, как у клипов: одна дорога к таймлайну, а не две. Колбэков
  // здесь не передают, но правило «в таймлайн ходят только через охрану» не должно иметь
  // исключения, к которому потом припишут второе.
  var capsTimeline = guardedTimeline(tl, 'субтитры сегмента ' + IR.segmentId);
  ...
  for (var g = 0; g < IR.captions.length; g++) {
    ...
    caps.appendChild(el);
    // ДВЕ СТРОКИ ВИДИМОСТИ (решение владельца R2). Полуинтервал T4: на кадре `end` группы
    // уже нет. Охранник — `test/captions-visibility.test.ts` (нарушение Н6 протокола):
    // удали эти строки руками, и он покраснеет на кадрах, а не промолчит.
    el.style.opacity = '0';
    capsTimeline.set(el, { opacity: 1 }, toSeconds(group.frames.start));
    capsTimeline.set(el, { opacity: 0 }, toSeconds(group.frames.end));
  }
