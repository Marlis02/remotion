...
export interface CacheManifestEntry {
  ...
  /**
   * Хэш композиции, из которой получены байты (`CACHE-01`); с `CACHE-02` — ДИАГНОСТИКА.
   *
   * ~~Без этой записи попадание засчитывалось бы правкой шаблона.~~ *(изменено: `CACHE-02`,
   * 2026-09-11.)* Теперь `bundleHash` — ВХОД `segmentKey` (`views/segment.json`), то есть
   * другой код шаблона даёт другой КЛЮЧ, и запись предыдущей реализации просто не находится.
   * Решений на этом поле больше нет ни одного: оно остаётся, потому что манифест читают
   * глазами, и «какой композицией снята запись» — законный вопрос, на который иначе нечем
   * ответить (из байтов `.mts` эта величина не выводится).
   */
