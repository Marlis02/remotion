 ...
 *
 * ЭКСПОРТИРУЕТСЯ РАДИ КЭША СТАДИЙ (`M-05`). ADR-0005 §1 называет `.cache/<stage>/` «CAS по
 * ключам», то есть раскладка там та же, и второй константы «2» в репозитории быть не должно:
 * разойдись они — и один каталог стал бы читаться двумя способами. Сам `blobPath` кэш НЕ
 * зовёт: он про `.store` и про бренд `Sha256`, а ключи стадий — не sha256 и живут в другом
 * корне.
 */
