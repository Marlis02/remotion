...
describe('`TS-01` — реестр отказывает, а не предупреждает', () => {
  // ~~Пять спеков фикстуры.~~ *(изменено: `E-07` — шесть; `E-02` — СЕМЬ.)* Порядок здесь
  // ЗНАЧИМ: он повторяет `TEMPLATE_LIBRARY`. ~~То есть порядок записей
  // `fixtures/minimal/direction/01-intro.yaml` плюс `grade@1` и `parallax25@1` в хвосте.~~
  // *(изменено: `TPL-01a`, 2026-09-09.)* **Теперь это БАЙТОВЫЙ порядок имён папок:** реестр
  // производный от листинга каталога, а прежний порядок был свойством чужого файла и из
  // каталога не выводился ничем.
  //
  // Реестр адресует по имени, и на поведение порядок не влияет — это ИЗМЕРЕНО, а не
  // предположено: реестр КОМПОЗИЦИИ (тот, что входит в `bundle.hash`) строится из
  // `request.ir.clips`, а не отсюда, поэтому смена порядка не сдвинула ни одного из двенадцати
  // `bundle.hash` запросов гейта. Тест держит порядок, чтобы перестановка была видна диффом.
  it('девять спеков библиотеки регистрируются, порядок — байтовый по имени', () => {
    ...
    expect(registry.specs).toHaveLength(9);
    // И порядок ИМЕННО байтовый, а не «как получилось»: сортировка своего же списка ничего
    // не меняет. Без этой строки тест держал бы снимок, а не правило.
    expect([...registry.names]).toEqual([...registry.names].sort((a, b) => (a < b ? -1 : a > b ? 1 : 0)));
  });
...
describe('`TS-01` — разрешение адреса', () => {
  ...
  it('неизвестный шаблон — отказ со списком зарегистрированных', () => {
    expect(() => registry.resolve('shaderBg@1')).toThrow(/Зарегистрированы: bed@1, captionEmphasis@1/);
  });
