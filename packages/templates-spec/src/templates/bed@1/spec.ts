...
//
// **МУЗЫКА МИКШИРУЕТСЯ С `X-02` (2026-09-12), И ЭТОТ ФАЙЛ — ТО МЕСТО, ГДЕ ЭТО НАЧИНАЕТСЯ.**
// ~~Долг №141: дорожка v1 = речь + тишины движка… `inPoint`/`gainDb`/`duckUnderSpeechDb`
// объявлены и типизированы, не более.~~ Теперь все три уезжают в микс четвёртой декларацией
// `declareAudio` — компилятор читать их по имени поля по-прежнему не вправе (`CP-07`), и это
// ровно тот путь, которым решён долг №119 у `declareDuration`. Правило «компилятор не
// выдумывает звук» не тронуто: микс складывает то, что объявил ВЫЗОВ, и ничего сверх.
...
export const bed1: TemplateSpec<BedParams> = {
  ...
  guidance:
    ...
    'второй ассет, которого шаблон не объявляет, и схема такую пару отвергает. Подложка ' +
    'ЗВУЧИТ: она укладывается в окно клипа с точки `inPoint`, зацикливается, если ассета не ' +
    'хватает на окно, идёт в дорожку с уровнем `gainDb` и уходит вниз ещё на ' +
    '`duckUnderSpeechDb` под каждой фразой диктора (плавно, по окнам речи). Оба значения ' +
    'отрицательные: тракт не поднимает уровень выше принесённого файлом.',
  ...
  // ЗВУК ВЫЗОВА (`X-02`). Три числа автора уезжают в микс как есть: точка входа, уровень и
  // глубина ухода под речь. `loop: true` — потому что подложка ПОДКЛАДЫВАЕТСЯ: окно клипа
  // задают якоря автора, и кончиться раньше него музыка не имеет права (иначе под второй
  // половиной сцены была бы тишина, которой никто не просил).
  //
  // `inPoint.offsetSamples`, а не `inPoint` целиком: миксу нужна ВЕЛИЧИНА, а `asset` внутри
  // точки схема уже сверила с `asset` подложки — второй ассет здесь невыразим.
  declareAudio: (params): AudioContribution | null => ({
    role: 'asset',
    // Сэмплы ПРОЕКТА как есть: `MediaTime` — единица, которую подложка знает про себя
    // (в отличие от `video@1`, который считает кадрами своего файла).
    inPoint: { unit: 'samples', value: params.inPoint.offsetSamples },
    gainDb: params.gainDb,
    duckUnderSpeechDb: params.duckUnderSpeechDb,
    loop: true,
    // Подложка не замирает никогда: паузы кадра — свойство видео, а не музыки.
    pauses: [],
  }),
  declareFonts: () => [],
  manifest,
