...
const manifest: TemplateManifest = {
  ...
  // Не измерялся ни одним спайком; заменяется измерением при `E-00`.~~ *(изменено: `H-06`,
  // 2026-08-29 — ИЗМЕРЕНО.)*
  //
  // **`FACT`, ИЗМЕРЕНО `H-06`: 29.727 мс/кадр ⇒ бюджет `Math.ceil` = 30.** Разность медиан —
  // 1783.6 мс на 60 кадров при шуме 70.7 мс. **ОЦЕНКА БЫЛА ЗАНИЖЕНА В ТРИДЦАТЬ РАЗ, И ЕЁ
  // ОШИБКА ПОУЧИТЕЛЬНА:** «собственной геометрии нет» верно и к делу не относится. Дорого не
  // движение, а КОМПОЗИТИНГ — полупрозрачный слой во весь кадр 1080×1920 смешивается с тем,
  // что под ним, на каждом кадре окна, и цена этого почти равна цене ресэмплинга фото у
  // `kenburns@1` (32.944). Слово «твин» описывает вход, а не работу растеризатора.
  //
...
export const flash1: TemplateSpec<FlashParams> = {
  ...
  guidance:
    'Короткая вспышка на дорожке `effect` — метка момента, а не украшение. Длительность ' +
    'задаёт САМ шаблон параметром `durationSamples`, поэтому `until` записи не нужен: она ' +
    'ставится одной точкой `at` на бит. Кривая затухания у него ровно одна и в `params` НЕ ' +
    'едет (`power3.out`): форма затухания есть свойство эффекта, а не места, куда его ' +
    'поставили. Не чаще одного раза на сцену и только на том слове, ради которого сцена ' +
    'написана.',
  declareAssets: () => [],
  declareFonts: () => [],
