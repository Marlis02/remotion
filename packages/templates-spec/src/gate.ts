...
//
// ПРАВИЛО «ЗАПИСЬ ГОДИТСЯ ИЛИ УСТАРЕЛА» ЖИВЁТ В ОДНОЙ ФУНКЦИИ — `gateStaleness` (`E-00`).
// Её зовут двое: этот вход **R12** (сборка) и команда `vpe template gate` (пересъёмка), а
// третьим станет `vpe build` (`L-01`). Две копии одного сравнения разъехались бы в первой же
// правке — ровно тот класс беды, что и долг №169 про склейку `GateMedia`.
//
// ПОЧЕМУ У НЕЁ ДВЕ ПОЛОВИНЫ И ОДНА ИЗ НИХ НЕОБЯЗАТЕЛЬНА. `engineFingerprint` описывает
// ОКРУЖЕНИЕ, `bundleHash` — КОД САМОГО ШАБЛОНА (sha256 перечня каталога композиции). Вторая
// половина проверяется только тем, кто композицию уже построил: на старте сборки ролика её
// ещё нет, а у команды гейта — есть. Вызывающий, который её знает, ОБЯЗАН подать: запись без
// `bundleHash` при известном фактическом — устаревшая ПО ПОСТРОЕНИЮ (поправка владельца П2),
// а не «проверка пропущена».
//
...
/** Фактическое состояние, против которого сверяется запись. */
export interface GateActual {
  /** Профиль, на котором собираются рендерить: `final` либо `draftHalf`. */
  readonly profileId: GateProfileId;
  /** `engineFingerprint` ЭТОЙ машины (ADR-0006 §3). Сравнение — строгим равенством строк. */
  readonly engineFingerprint: string;
  /**
   * Фактический `bundle.hash` композиции, если вызывающий её построил.
   *
   * `undefined` — не «всё в порядке», а «этой половиной вопрос не задан»: на старте сборки
   * ролика композиции ещё нет. Команда гейта и `vpe build` (`L-01`) подают его всегда.
   */
  readonly bundleHash?: string;
}

/** Запись, как она лежит В ФАЙЛЕ: `GateRecord` плюс `bundleHash` (форма `template-gates/1`). */
export interface GateCandidate {
  readonly gate: GateRecord;
  /** `undefined` — запись СТАРОЙ формы, без `bundleHash` (см. `gateStaleness`, П2). */
  readonly bundleHash?: string;
}

/**
 * **Годится ли запись гейта для пары.** `null` — годится; строка — причина, дословно
 * попадающая в текст отказа **R12** и в вывод команды.
 *
 * ПОРЯДОК ПРОВЕРОК — профиль, окружение, класс, код шаблона, — и он взят НЕ с потолка: ровно
 * в этом порядке спрашивал `assertBuildMayStart` до перевода на эту функцию (`TS-01`).
 * Сохранён он затем, чтобы перевод не менял ни одного текста отказа: правило переехало в одно
 * место, поведение осталось прежним (поправка владельца П4 — счёт тестов R12 до и после).
 */
export function gateStaleness(candidate: GateCandidate, actual: GateActual): string | null {
  const { gate } = candidate;

  if (gate.profileId !== actual.profileId) {
    return (
      `запись снята на профиле \`${gate.profileId}\`, а пара сборки — \`${actual.profileId}\`. ` +
      'Гейт снимается на каждом профиле отдельно (решение владельца 12, RM1)'
    );
  }

  if (gate.engineFingerprint !== actual.engineFingerprint) {
    return (
      `запись гейта снята на другом окружении: в манифесте \`${gate.engineFingerprint}\`, ` +
      `у сборки \`${actual.engineFingerprint}\`. Область действия гейта — одна машина, один ` +
      'набор профилей, одна композиция (ADR-0008); при смене любого из трёх гейт переснимается'
    );
  }
  ...
  if (actual.bundleHash !== undefined) {
    if (candidate.bundleHash === undefined) {
      return (
        'запись СТАРОЙ формы: `bundleHash` в ней нет, а композиция известна. Сверить нечем, ' +
        'и это устаревание по построению (поправка владельца П2, `E-00`): отпечаток ' +
        'описывает окружение и молчит о коде самого шаблона — правка `mountSource` без ' +
        'смены версии оставила бы такую запись «действующей»'
      );
    }
    if (candidate.bundleHash !== actual.bundleHash) {
      return (
...
function rejectionFor(
  ...
  // ПРАВИЛО — В `gateStaleness`, ЗДЕСЬ ТОЛЬКО АДРЕС. Класс, окружение и (когда вызывающий её
  // знает) композиция сверяются одной функцией; сюда добавляется команда пересъёмки, потому
  // что она зависит от ИМЕНИ шаблона, а не от правила.
  // `bundleHash` НЕ ПОДАЁТСЯ, и это не забывчивость: на старте сборки композиции ещё нет —
  // её материализует адаптер на каждый сегмент. Половину «код шаблона» задаёт тот, кто
  // композицию построил: команда `vpe template gate` (сейчас) и `vpe build` (`L-01`).
  const stale = gateStaleness(
    { gate },
    { profileId: pair.profileId, engineFingerprint: pair.engineFingerprint },
  );
  if (stale !== null) {
    return {
      template,
      why: `${stale}. Переснимите гейт: \`vpe template gate ${template} --profile ${pair.profileId}\``,
    };
  }
