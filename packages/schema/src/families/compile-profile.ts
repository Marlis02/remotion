...
const CaptionsSchema = z
  .object({
    ...
    // ВТОРОЙ потолок группы — по числу символов (ADR-0003 «Субтитры (M6)»: «шаблону субтитров
    // запрещены перенос строки и автоуменьшение кегля — группа, не помещающаяся в строку,
    // дробится числом слов, а не сжимается вёрсткой»). Считается по display-тексту группы
    // вместе с пробелами.
    //
    // УМОЛЧАНИЯ У НЕГО НЕТ НАМЕРЕННО (решение владельца 2026-08-26): умолчание — это число,
    // которого никто не решал, а это число зависит от шрифта и кегля и подобрано измерением
    // (`CP-02`). Профиль без поля обязан отвергаться, а не собираться «как-нибудь».
    maxGroupChars: z.int().positive(),
  })
  .strict()
