...
const TemplateRecordSchema = z
  .object({
    ...
    template: identifier(),
    /**
     * **Имя пресета — `params`, сохранённые под именем в папке шаблона** (`TPL-01b`,
     * 2026-09-10; правка семейства — по точечному разрешению владельца).
     *
     * Грамматику имени (`^[a-z][a-z0-9-]*$`) нормирует, как и у `template`, сторона шаблона —
     * `templates-spec/src/presets.ts`; здесь идентификатор, и не больше. Разворачивает пресет
     * КОМПИЛЯТОР до `paramsSchema` (`compile/src/timeline/contract.ts`), поэтому ни в IR, ни
     * в один ключ кэша, ни в seed это поле не попадает: смена имени пресета при тех же числах
     * не имеет права переснять ни одного кадра.
     */
    ...
    //
    // **НЕОБЯЗАТЕЛЬНЫ ТОЛЬКО ПРИ `preset`** (`.refine` ниже): запись, назвавшая пресет, вправе
    // не повторять его чисел, а запись без пресета обязана дать `params` сама.
    params: z.record(z.string(), JsonValueSchema).optional(),
  })
  .strict()
  .refine((record) => record.preset !== undefined || record.params !== undefined, {
    message:
      'запись шаблона обязана нести `preset` либо `params` (можно оба — тогда `params` ' +
      'накладываются ПОВЕРХ пресета по верхнему уровню ключей). Запись без обоих не ' +
      'называет ни одного значения, и «шаблон с параметрами по умолчанию» движок не знает: ' +
      'умолчания живут в схеме шаблона, а не в пустоте на месте вызова',
  });
