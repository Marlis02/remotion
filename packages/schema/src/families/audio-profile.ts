...
/**
 * Микс дорожки ролика (`X-02`, решение владельца 2026-09-12 «микс делаем»).
 *
 * **ПОЧЕМУ ОБА ПОЛЯ С УМОЛЧАНИЕМ, А НЕ ОБЯЗАТЕЛЬНЫЕ.** Профиль звука лежит в шести проектах
 * репозитория, и пять из них были написаны до микса. Обязательное поле сделало бы их
 * нечитаемыми — то есть правка схемы уронила бы сборку ролика, в котором нет ни одной
 * подложки. Умолчание здесь — не «на всякий случай», а утверждение: **микс включён, пока
 * проект не сказал обратного**, и это ровно то поведение, которого владелец просил.
 *
 * `enabled: false` — ПРИБОР СРАВНЕНИЯ, а не режим на каждый день: дорожка при нём побайтно
 * равна дорожке до `X-02`, и это единственный способ сверить старую запись с новой.
 *
 * `duckRampMs` — длина спуска подложки ПЕРЕД фразой и подъёма ПОСЛЕ неё. 120 мс —
 * `INFERENCE`, а не измерение: короче 50 мс рампа сама слышна щелчком, длиннее 200 мс —
 * подложка не успевает уйти под короткую фразу. Измерения в репозитории нет; величина
 * названа ручкой профиля именно затем, чтобы её можно было менять без правки кода, и её
 * основание помечено в отчёте `X-02`.
 */
const MixSchema = z
  .object({
    // ПОРЯДОК ПОЛЕЙ — АЛФАВИТНЫЙ, И ЭТО НЕ ВКУС: каноническая форма `S-02` сортирует ключи
    // блока, и порядок shape'а обязан с ней совпасть, иначе `checkCanonical` на своём же
    // выходе краснеет (охранник `canonical-form.test.ts`).
    duckRampMs: z.int().nonnegative().default(120),
...
export const AudioProfileSchema = z
  .object({
    ...
    // Микс ролика (`X-02`). Блок целиком необязателен: проект, написанный до микса, читается
    // как «микс включён, рампа 120 мс» — см. разбор у `MixSchema`.
    mix: MixSchema.default({ duckRampMs: 120, enabled: true }),
    /**
