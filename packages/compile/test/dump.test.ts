...
describe('CP-01 — канонический дамп', () => {
  ...
  it('диффится ПОСТРОЧНО: у каждого клипа своя строка, и интервалы полуоткрыты', async () => {
    ...
    // 1 шапка + 7 строк дорожек + клипы + 1 строка кандидатов + кандидаты + 1 строка якорей + якоря.
    expect(lines).toHaveLength(1 + 7 + items + 1 + timeline.cutCandidates.length + 1 + timeline.anchors.length);
