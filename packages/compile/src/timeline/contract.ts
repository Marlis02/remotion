...
function contractOf(
  ...
  for (const ref of spec.declareAssets(value)) {
    ...
    // ═══ ВИД ФАЙЛА ПРОТИВ ВИДА, КОТОРОГО ЖДЁТ РОЛЬ (`ASSET-01` §2.3, 2026-09-10) ═══
    // ДО ЭТОЙ ПРОВЕРКИ ЗДЕСЬ БЫЛА ДЫРА, И ОНА ИЗМЕРЕНА: alias разрешался в sha, пара
    // `{sha, role}` уезжала в IR, и ЧТО это за файл не спрашивал никто. Видео под `still@1`
    // доезжало молча до рендерера и падало ТАМ — в подпроцессе, сообщением про `<img>`, в
    // трёх стадиях от записи, которую надо чинить. С приходом вида `video` (`VID-01`) такой
    // вход перестал быть гипотетическим: `vpe asset add` кладёт видео в тот же каталог и под
    // такой же alias, что и фотографию.
    //
    // ПОЧЕМУ ПРОБЛЕМА, А НЕ ИСКЛЮЧЕНИЕ: это ошибка АВТОРА в записи режиссуры, и приходит она
    // пачками (перепутал alias — покраснели все записи сцены). Тот же довод, что у остальных
    // проверок этой функции.
    const record = input.catalog.records.get(sha256);
    const expected = ref.kind ?? DEFAULT_ASSET_KIND;
    if (record !== undefined && record.kind !== expected) {
      problems.push({
        address: where,
        message:
          `\`${template}\` ждёт под ролью \`${ref.role}\` файл вида \`${expected}\`, а alias ` +
          `\`${ref.alias}\` указывает на запись вида \`${record.kind}\` (\`${sha256}\`). ` +
          'Вид ассета — свойство ФАЙЛА, измеренное при `vpe asset add`, а не мнение записи: ' +
          'подставить сюда другой вид значило бы попросить рендерер нарисовать видео тегом ' +
          '`<img>` — и узнать об этом в подпроцессе, а не здесь. Возьмите alias нужного вида ' +
          '(`vpe asset list --project <кат>` печатает вид у каждого)',
      });
      continue;
    }
