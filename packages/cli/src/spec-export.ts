...
const EXAMPLES: readonly SpecExportExample[] = [
  ...
  },
  {
    ...
    note:
      ...
      'прозрачное окно; слой на ТОМ ЖЕ `z` видео закроет. Второе: звук видео попадает в ' +
      'ролик, только если его попросить (`audio`: `off` — умолчание, `full` — звук ровно, ' +
      '`duck` — звук уходит вниз под каждой фразой диктора на −6.3 дБ); у видео без звуковой ' +
      'дорожки `full`/`duck` — отказ с адресом записи. Третье: видео, кончившееся раньше окна, ' +
      'держит последний кадр либо идёт заново с нуля — это выбирает `loop`, и звук ' +
      'повторяется вместе с картинкой. ЧЕТВЁРТОЕ (`VID-02c`): окно врезки держит ПРОПОРЦИЮ ' +
      'ВИДЕО — `size` задаёт ширину, высота считается от файла, — поэтому `fit` внутри ' +
      'врезки не меняет ничего и просить его там незачем. ПЯТОЕ: окно умеет ехать (`move`) и ' +
      'скругляться (`radius`); переезд линеен, вместе с `zooms` и при `fit: "contain"` он ' +
      'отвергается.',
    record: {
      recordId: 'd1f0a5c3',
      at: { kind: 'anchor', anchor: 'b:img-street-1' },
