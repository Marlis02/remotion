...
export interface TakeProvenance {
  ...
  /**
   * СТАВКА тарифа на дату генерации: сколько единиц списывается за один отправленный
   * code point (`V-06`; ADR-0010 §2 требует хранить её **рядом** с `planTierAtGeneration`).
   *
   * ЛЕЖИТ РЯДОМ, А НЕ ВНУТРИ `billedUnits`, И ЭТО НЕ ФОРМАЛЬНОСТЬ. «Сколько отправлено» —
   * величина, вычислимая офлайн из самого дубля (code points `spokenText`); «сколько стоило» —
   * функция тарифа, которого в дубле нет. Склеенные в одно имя, они дают число, которое нельзя
   * ни проверить, ни пересчитать: `FACT` (SP-2b.7) на Free списывалось `1.00 × code points`, а
   * на Creator — `round(codePoints × 0.55)` покалльно.
   *
   * `null` — «ставка не объявлена», и это ЗАКОННОЕ значение: так пишут дубль провайдеры,
   * которые ничего не отправляют (`tts:mock@1`, `billedUnits: 0`), и так остаётся у живого
   * дубля, снятого без снимка аккаунта. Ноль сюда писать нельзя — «ставка 0» означало бы
   * «дубль бесплатен», а это утверждение о деньгах, которого никто не делал.
   *
   * КОНСТАНТОЙ В КОДЕ ЭТО ЧИСЛО НЕ ЖИВЁТ (ADR-0010 §2 дословно): `UNKNOWN` (SP-2b.7) — откуда
   * берётся 0.55, в ответах API нет (`token_cost_factor` модели равен 1 в обоих снимках
   * аккаунта), и стабильность множителя во времени и по моделям не измерялась. Значение
   * приезжает снимком аккаунта, а охранник `assertBilledRate` (`providers/usage.ts`) сверяет
   * его с окном `usage/character-stats` при каждом живом прогоне.
   */
