...
export interface TakeHealth {
   ...
   * именно её знак и есть признак «таймкоды вышли за пределы фактического PCM», по которому
   * дубль отвергается. `asSamples` отрицательные отвергает (`S-01`), то есть бренд здесь
   * сделал бы невыразимым ровно то состояние, ради обнаружения которого поле заведено.
   */
