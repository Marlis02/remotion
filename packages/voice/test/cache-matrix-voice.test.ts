...
describe('K1/upstream — поле вне view действует через НАЗВАННОЕ поле проекции (долг №87)', () => {
  ...
  it('применимая роль ВСЕГДА перекрывает голос проекта: `voice_id` в схеме роли обязателен', () => {
    // Свойство, найденное матрицей. Пока роль применима, `project.yaml → voice.voiceId` на
    // ключ не влияет ничем — и это не дефект, а следствие того, что `voice-roles/1` требует
    // `voice_id` у каждой записи. Записано утверждением, чтобы обратное изменение схемы
    // (сделать поле необязательным) было видно как смена поведения ключей.
