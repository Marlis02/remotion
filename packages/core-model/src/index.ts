...
// ТРЕТИЙ АДРЕСНЫЙ БЛОК РЕЭКСПОРТА — решение владельца 2026-08-26 (`CP-02`), по образцу двух
// выше. Кому: пакету `@vpe/compile`, вход для `frameStartSample`. Зачем: сама функция уже
// экспортирована и потребляется, но её параметр — бренд `Frames`, а `@vpe/schema` из
// `compile` по карте ADR-0009 не резолвится вовсе (`packages/compile/node_modules/@vpe/` —
// четыре симлинка). Без конструктора у потребителя остаются только каст (запрещён `S-01`
// долгом №3 и охранником `tests/lints/brand-casts.test.ts`) и вторая копия формулы T2
// `floor(f · sampleRate · fpsDen / fpsNum)` — а вторая копия формулы хуже любой строки
// реэкспорта. Сам `@vpe/schema` этой строкой не изменяется ни символом.
export { asFrames, type Frames } from '@vpe/schema';
