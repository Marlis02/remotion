...
export interface IrAssetRef {
  ...
  /**
   * Чем ассет служит шаблону — строка, которую вернул `declareAssets(params)` спека
   * (`templates-spec`). *(Уточнено: `CP-07`, 2026-08-28.)* До `CP-07` роль была только у
   * порождённой `[img:]`-записи и ставилась компилятором из имени параметра (ADR-0002 §4,
   * долг №138); теперь её называет ШАБЛОН, и у `still@1` это та же строка `'asset'` — форма
   * ссылки не изменилась ни полем, ни значением.
   */
  readonly role: string;
}

/**
 * Шрифт, на который ссылается клип: sha, СЕМЕЙСТВО и роль — как в `SegmentRenderRequest.fonts`
 * (ADR-0008 «Контракт»), плюс роль, которой запрос не несёт.
 *
 * *(Добавлено: `CP-07`, 2026-08-28, решение владельца A. Закрывает долг №139: до этой правки
 * `RenderIrSegment.fonts` был `readonly never[]` — пометкой типом «наполнителя нет».)*
 *
 * ПУТИ ЗДЕСЬ НЕТ — по той же причине, что у `IrAssetRef`: `path` в запросе рендерера есть
 * свойство конкретного прогона (content-addressed путь во `tmpDir`), а IR — свойство
 * произведения. Путь подставляет адаптер (`H-01`), он же материализует каталог композиции.
 *
 * `family` — ИЗМЕРЕННОЕ `intrinsic.family` записи `asset-record/1` (ADR-0005 §1), а не то,
 * что назвал шаблон: `FontRef.family` спека необязателен (шрифт канала не выбран, №13), и
 * шаблон, вписавший семейство, увёз бы временный выбор в `engineFingerprint` вместе с первым
 * гейтом. `role` — то, что шаблон объявил (`captionEmphasis@1` → `'caption'`).
 *
 * `weight` из формы `SegmentRenderRequest.fonts` здесь НЕ ЗАВОДИТСЯ: `asset-record/1` несёт
 * `intrinsic.subfamily` (`'Bold'`), а не числовой вес, и перевод одного в другой был бы
 * правилом, которого не пишет ни один документ. Долг с адресом `H-01`.
 */
export interface IrFontRef {
  readonly sha256: Sha256;
  /** `intrinsic.family` записи шрифта: `'DejaVu Sans'` на `fixtures/minimal`. */
  readonly family: string;
  /** Роль, объявленная `declareFonts(params)`: чем шрифт служит шаблону. */
  readonly role: string;
}
...
export interface IrClip {
  ...
  /**
   * Шрифты, объявленные `declareFonts(params)` шаблона — по одному на роль.
   *
   * *(Добавлено: `CP-07`, 2026-08-28.)* Лежат у КЛИПА, а не только у сегмента, по той же
   * причине, по которой там лежат `assets`: роль связывает файл с ТЕМ шаблоном, который его
   * попросил, и на уровне сегмента эта связь теряется. Список сегмента — их объединение.
   */
   ...
   * различает их только `purpose`» (`model/seed.ts`). *(Уточнено: `CP-07`, 2026-08-28.)*
   * Перечень `purpose` объявляет МАНИФЕСТ шаблона (`manifest.purposes`), и с этого места
   * карта строится по нему, а не по id шаблона (долг №135 закрыт). На `fixtures/minimal`
   * карта ПУСТА у всех клипов: случайности не требует ни один из пяти шаблонов — это
   * измерение (`TS-01` §5 п. 2), а не заглушка.
   *
...
export interface RenderIrSegment {
  ...
  /**
   * Ассеты сегмента — ОБЪЕДИНЕНИЕ ассетов его клипов, отсортированное по `(sha256, role)`.
   *
   * *(Добавлено: `CP-07`, 2026-08-28, решение владельца A.)* Форма адресована рендереру:
   * `SegmentRenderRequest.assets` (ADR-0008 «Контракт») — список УРОВНЯ ЗАПРОСА, из которого
   * адаптер материализует каталог композиции, и собирать его обходом клипов означало бы
   * второе место, где список запроса определяется (**R3** требует ЕДИНСТВЕННОГО источника).
   *
   * ОБЪЕДИНЕНИЕ, А НЕ КОНКАТЕНАЦИЯ: один и тот же ассет в двух клипах сегмента — один файл в
   * запросе. Пара `(sha256, role)` уникальна; один sha в двух ролях — две строки, потому что
   * роль есть часть того, ЧТО просит шаблон.
   *
   * СОРТИРОВКА, А НЕ ПОРЯДОК КЛИПОВ, и это не косметика: порядок клипов зависит от `z` и
   * `sourceOrdinal`, то есть от того, что стоит выше по тексту, — и AC4-b («тот же сегмент в
   * двух проектах побайтово равен») сломался бы от вставки слоя, файлов не меняющей.
   */
  readonly assets: readonly IrAssetRef[];
  /**
   * Шрифты — файлами с checksum (ADR-0008 «Гарантии входа»); объединение по клипам, порядок
   * тот же `(sha256, role)`.
   *
   * ~~`never[]` — ПОМЕТКА ТИПОМ, а не забытое поле: список наполняет `declareFonts` шаблона
   * (`TS-01`), которого нет.~~ *(изменено: `CP-07`, 2026-08-28 — наполнитель появился, долг
   * №139 закрыт.)* На `fixtures/minimal` непуст ровно у `seg:turn`: единственный шаблон,
   * просящий шрифт, — `captionEmphasis@1` (роль `caption`), и стоит он на `b:close`.
   */
  readonly fonts: readonly IrFontRef[];
}
