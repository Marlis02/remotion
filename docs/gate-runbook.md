...
**Что получится.** ~~Четыре~~ ~~пять~~ ~~ШЕСТЬ~~ **ВОСЕМЬ** файлов
...
2026-08-31; седьмой — `video@1`, добавлен `VID-02a`, 2026-09-11; восьмой — `kineticType@1`,
добавлен `KT-01`, 2026-09-12. Девятый шаблон библиотеки, `bed@1`, записей гейта НЕ имеет и
иметь не может: он аудио-домена и в `RenderIR.clips` не попадает — долг №189. Что список
шаблонов без записей закрыт именно им, стережёт
[`gates-on-current-bundle.test.ts`](../packages/templates-spec/test/gates-on-current-bundle.test.ts).)*

**И ЕЩЁ ОДНО, ЧТО СТОИТ ЗНАТЬ ДО ПЕРВОЙ КОМАНДЫ** *(`KT-01`, 2026-09-12)*: тот же охранник
сверяет `bundleHash` КАЖДОЙ коммитнутой записи с `bundle.hash` запроса её папки. То есть
вопрос «не устарела ли запись» теперь задаётся на ПРОГОНЕ ТЕСТОВ, а не только глазами
владельца, и §0-bis («после `L-01` записи устарели — переснять все восемь») в следующий раз
начнётся с красного теста, а не с чтения отчёта. Это половина долга №196: полный требует
сверки на СБОРКЕ, то есть подачи `bundleHash` во вход **R12**.
...
## 0-ter. ГЕЙТЫ `draftHalf` ПЕРЕСНЯТЫ 2026-09-11 (`FIX-02`, долг №265)

Композиция ЧЕРНОВОГО профиля изменилась: масштаб раскрывается на `.layer`, а не теряется вовсе,
поэтому `bundleHash` всех шести запросов `draftHalf` сдвинулся, а прежние записи описывали кадр,
которого больше нет (в нём была видна только левая верхняя четверть композиции). Шесть записей
сняты заново шестью командами §1 — все **PASS, N = 3**, отпечаток `3217a4cf…`. **Записи `final`
не тронуты ни байтом** (доказано побайтовым равенством файлов и записей — [отчёт
`FIX-02`](impl/FIX-02/report.md) §4), и переснимать их не надо.
...
## 4-ter. КАК ДОБАВИТЬ СЛЕДУЮЩИЙ ШАБЛОН (`TPL-01a`, 2026-09-09; проверено на девятом — `KT-01`, 2026-09-12)
...
| `templates-spec/…/<id>@1/gate-case.json` | **случай гейта**: `note`, `captions` и `clips` (шаблон, `params`, `z`, `withAsset`/`withFont`/`withLayers`). Из него порождаются ОБА запроса, и его же читает браузерный гейт — копий больше нет *(добавлено: `TPL-01b`, долг №193)* | да, если гейт снимается |
| `templates-spec/…/<id>@1/presets/<имя>.json` | **пресеты**: `{ "note": "…", "params": { … } }`, имя файла = имя пресета (`^[a-z][a-z0-9-]*$`). Каждый прогоняется `paramsSchema` шаблона при загрузке каталога *(добавлено: `TPL-01b`)* | нет |
| `templates-spec/…/<id>@1/demo/demo.json` | **демо**: заголовок, пояснение, проза, ассеты и по записи режиссуры на каждый пресет. Разворачивает его в собираемый проект команда `vpe template demo` *(добавлено: `TPL-01c`)* | **да** — шаблон не считается готовым без демо |
| `templates-spec/…/<id>@1/demo/assets/…` | свои байты демо, если ассетов гейта не хватает (сегодня только звук `bed@1`) | если демо просит своё |
...
(`renderer-hyperframes/test/fixture.ts`) — это единственное место сверх папки, и оно известно.
Вторая копия того же списка живёт в `templates-gate.test.ts` (долг №193).~~ *(изменено:
`TPL-01b`, 2026-09-10 — долг №193 закрыт.)*

**Случай гейта — ФАЙЛ ПАПКИ `<id>@1/gate-case.json`** (шаг 1). Мест сверх папки не осталось
ни одного: `GATE_REQUEST_CASES` фикстуры и `CASES` браузерного гейта читают его, а не
повторяют. Значит «владелец снял запись на композиции, которую браузерный тест не мерил» стало
невыразимо.
...
### Шаг 3-бис — пресеты (необязательно, но лучше сразу)

Пресет — `params` шаблона, сохранённые под именем: автор пишет `preset: "<имя>"` вместо чисел.
Файл кладётся руками, `pnpm build` ему не нужен:

```bash
cat > packages/templates-spec/src/templates/<id>@1/presets/<имя>.json <<'JSON'
{ "note": "для чего он и откуда взяты числа — одной фразой", "params": { } }
JSON
```

Правила ровно три: имя файла и есть имя пресета (`^[a-z][a-z0-9-]*$`); `note` обязателен — он
уходит в `vpe spec export`, и без него имя нечем выбрать; `params` обязаны пройти
`paramsSchema` СВОЕГО шаблона — кривой пресет краснеет при загрузке каталога, а не на первой
записи, которая его позовёт. Числа берутся из СОБРАННОГО ролика, а не из головы.

### Шаг 3-тер — демо (`TPL-01c`, 2026-09-10). **ОБЯЗАТЕЛЬНО**

**Шаблон не считается готовым без демо** — требование владельца, и оно не про красоту: гейт
отвечает «повторяется ли шаблон побайтово», пресеты — «какими числами его зовут», а на
вопрос «что он вообще делает» до демо отвечал только чужой ролик в `examples/`.

Демо — ОДИН файл в папке шаблона:

```bash
cat > packages/templates-spec/src/templates/<id>@1/demo/demo.json <<'JSON'
{
  "title": "<id>@1 — одной строкой, что показано",
  "note": "какие пресеты идут и почему именно так; откуда взяты ассеты",
  "text": "# chapter: main\n\n## scene: demo\n\n[img: pattern] Проза демо. [beat: one] Второе предложение.\n",
  "assets": [
    { "alias": "pattern", "file": "../gate-requests/assets/pattern-32.png",
      "kind": "image", "intrinsic": { "width": 32, "height": 32 } }
  ],
  "fonts": [],
  "records": [
    { "at": { "kind": "anchor", "anchor": "b:one" },
      "until": { "kind": "anchor", "anchor": "sc:demo" },
      "track": "visual", "z": 10, "preset": "<имя пресета>" }
  ]
}
JSON
node packages/cli/dist/bin/vpe.js template demo <id>@1 --profile draftHalf
```

Правила, которых ровно шесть:

1. **по записи на КАЖДЫЙ пресет** — охранник требует равенства множеств «показано» и
   «объявлено», а не включения: пресет, которого демо не показывает, есть ручка, которую
   владелец не увидит ни разу;
2. **`recordId` не пишется** — команда выводит его детерминированно из имени шаблона и
   номера записи. Случайность означала бы разный `sha256` финала на каждом прогоне;
3. **`sha256` ассета не пишется** — команда считает его по байтам файла;
4. **`file` резолвится от папки `demo/`** и вправе выйти за неё (`../gate-requests/assets/…`):
   ассеты запросов гейта уже в git с ясными правами, и второй копии тех же байтов быть не
   должно. Свои байты кладутся в `demo/assets/` — так сделано только у `bed@1`, которому
   нужен ЗВУК, а звука среди ассетов гейта нет по построению;
5. **`template` в записи необязателен** — умолчание — шаблон своей папки. Называть его явно
   нужно только ОСНОВАНИЮ (`still@1` под `grade@1`: красить поверх пустоты нечем);
6. **проза без шапки `source-dialect/1`** — её ставит команда.

Проверить, что каталог видит демо:

```bash
node packages/cli/dist/bin/vpe.js template list          # колонка `demo`, последняя строка — предупреждение
node packages/cli/dist/bin/vpe.js template demo --all    # все демо каталога подряд
```

Забыть этот шаг нельзя молча: `packages/cli/test/template-demo.test.ts` требует, чтобы демо
было у КАЖДОГО шаблона каталога, и краснеет именем того, у кого его нет.
...
**ПОРЯДОК МЕЖДУ ШАГОМ 3 И ШАГОМ 4 ЗНАЧИМ, И ЦЕНА НАРУШЕНИЯ ИЗМЕРЕНА** *(`KT-01`, 2026-09-12)*.
Запросы гейта — ПРОИЗВОДНЫЕ от кода шаблона: любая правка `impl.ts` или `spec.ts` двигает
`bundle.hash`, и записи, снятые до неё, становятся «снятыми на другой композиции». Живьём это
выглядит так: гейт отказывает `R2` — «`bundle.hash` запроса — `a92f2de8…`, а каталог
композиции, собранный из полей ЭТОГО ЖЕ запроса, имеет `4f09cf8b…`». Лечится повторением шага
3 (`VPE_GATE_REQUESTS_UPDATE=1`), а не правкой файла руками. В `KT-01` записи снимались
ДВАЖДЫ по этой причине: между первой и второй съёмкой у двух пресетов снизился кегль.

**ЗАПИСИ ПРЕЖНИХ ШАБЛОНОВ НЕ СДВИНУЛИСЬ И НА ЭТОТ РАЗ.** Тот же опыт, что у `E-07` и `E-02`,
повторён на девятом шаблоне и дал тот же ответ: `git status --porcelain
packages/templates-spec/src/templates/` показал РОВНО новую папку и `templates/index.ts`. Условие
то же самое и оно названо в `E-07`: `runtime.js` задача не трогала, версия реестра реализаций
не менялась, а композиция несёт только ИСПОЛЬЗОВАННЫЕ шаблоны (`materialize.ts`). Именно ради
сохранения этого условия `kineticType@1` читает субтитры из `window.__VPE_IR`, а не из `ctx`
(долг №292).
