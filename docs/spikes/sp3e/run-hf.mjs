/**
 * SP-3e: ОДИН прогон HyperFrames в отдельном процессе.
...
const result = {
  schema: 'sp3e-run/1', runId: cfg.runId, renderer: 'hyperframes', status: 'RUNNING',
  startedAt: new Date(T0).toISOString(),
  config: {profile: 'final', workers: cfg.workers, gpu: cfg.gpu ?? 'sw', project: cfg.project ?? 'src/hyperframes', outputPath: cfg.outputPath},
...
try {
  ...
  const args = ['render', cfg.project ?? 'src/hyperframes', '-o', cfg.outputPath, '--workers', String(cfg.workers),
