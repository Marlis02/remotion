...
# PROGRESS — CP-04fix (литеральный NUL в исходниках) · старт 2026-08-27T00:44+06:00
План: 4 шага — корректный скан, замена, охранник, отчёт.
- `00:44` [1/4] находка владельца проверена. Идиома `grep $'\x00'` в bash НЕ работает: шаблон вырождается в пустой и совпадает с каждой строкой (отсюда «список из четырёхсот файлов»). Скан python по байтам: **5 NUL в 3 файлах** — `render-ir/records.ts` (2, мой), `core-model/anchors/sync.ts` (1, `C-04`), `schema/write.ts` (2, `S-02`). Все пять — один приём: `U+0000` как невозможный в данных разделитель
- `00:48` [2/4] готово — пять байтов заменены на экранированную форму во всех трёх файлах; в `records.ts` дописано объяснение, зачем разделитель именно `U+0000` и почему он записан экранированно. Скан по `packages/`+`tests/`+`fixtures/`: **0 NUL**
- `00:55` [3/4] готово — охранник `tests/lints/nul-in-sources.test.ts` (3 теста). ПРИ ЕГО НАПИСАНИИ МЕХАНИЗМ ВОСПРОИЗВЁЛСЯ: экранированная запись, попадая в файл, снова стала литеральным байтом (4 штуки) — правил программно. Охранник покраснел на себе же до правки
- `00:59` [4/4] готово — прогоны: `typecheck`/`lint` зелёные, **102 файла / 1679 тестов**. Тест порядка `records` зелёный БЕЗ правки — поведение тождественно
**Итог:** 5 литеральных NUL в 3 файлах из 3 разных сессий заменены на `\u0000`; охранник заведён; `packages/` = 0.
Измерено попутно: git считает файл бинарным по NUL **в первых 8000 байтах** — `records.ts` (4645) и `sync.ts` (5013) выпадали из ревью, `write.ts` (21396) нет.
Детали — `docs/impl/CP-04/report.md` §13.
